# BambooEat | Geyser Extension

The Geyser Extension gives Bedrock players the full bamboo eating experience, matching Java Edition exactly.
...
> **Java players** get the eating animation via [ProtocolLib](https://github.com/dmulloy2/ProtocolLib) (optional soft dependency on the main plugin).
> **Bedrock players** get it via this Geyser Extension.
> The two systems are fully independent neither interferes with the other.
...
The extension maps bamboo to a food item by detecting the `minecraft:food` data component on the item stack. BambooEat (on the backend server) is what attaches that component Geyser at the proxy level sees it when forwarding item packets to the Bedrock client.
